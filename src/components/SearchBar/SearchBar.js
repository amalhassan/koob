...
const SearchBar = ({setQuery, setType}) => {
  ...
  return (
    <Flex w={'100%'} justifyContent={{base:'center', sm:'flex-start'}} alignItems={'center'} mt={'2rem'} columnGap={'10px'}>
      <form id="explore-form" onSubmit={handleSubmit}>
        <FormLabel hidden={true} htmlFor={search}>Search</FormLabel>
        <Input type='text' placeholder={"Explore"} w={{base: '65%', sm:'33%'}} borderColor={'koobBlack'} p={2} h={{base: '1.8rem', sm: '2rem'}} focusBorderColor='koobAccentGold' name="search" id='search' onChange={(e) => setSearch(e.currentTarget.value)}/>
